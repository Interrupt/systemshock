...
errtype audiolog_play(int email_id) {
    ...
    begin_wait();
    // MaxMem(&dummy);							// Compact heap before loading the alog. - DG: I don't think we need that
    // anymore
    ...
    /*
       ...
       palog = MoviePrepareRes(AUDIOLOG_BASE_ID + email_id, audiolog_buffer, sizeof(audiolog_buffer),
       AUDIOLOG_BLOCK_LEN); sdp=snd_sample_parms(palog->as.smp_id); sdp->vol=curr_alog_vol; sdp->pan=ALOG_PAN;
       sdp->snd_ref=0;
