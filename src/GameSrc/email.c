...
void mfd_emailmug_expose(MFD *mfd, ubyte control) {
    ...
    if (control & MFD_EXPOSE) // Time to draw stuff
    {
        ...
        parse_email_mugs((char *)RefGet(MKREF(msg, MUGSHOT_IDX), FORMAT_RAW), &mcolor, mnums, FALSE);
        for (mid = 0; mid < NUM_MFDS; mid++)
...
void select_email(int num, uchar scr) {
    ...
    mug_num = atoi((char *)RefGet(MKREF(id, MUGSHOT_IDX), FORMAT_RAW));
    ResUnlock(id);
...
void read_email(Id new_base, int num) {
    ...
    if (current_email_base == EMAIL_BASE_ID) {
        ...
        mug_num = atoi((char *)RefGet(MKREF(current_email_base + num, MUGSHOT_IDX), FORMAT_RAW));
        ...
        if (mug_num >= BASE_VMAIL) // video email
        {
            ...
            if (current_email != EMAIL_INACTIVE) {
                ...
                ushort mnums[NUM_MFDS];
                uchar grab = TRUE;
                parse_email_mugs((char *)RefGet(MKREF(current_email_base + num, MUGSHOT_IDX), FORMAT_RAW), NULL, mnums, TRUE);
