...
Q e0, e1, e2, e3, // For speed plus beauty!
    ed0, ed1, ed2, ed3, alpha_dot, beta_dot, gamma_dot;

Q object12, object13;

//		For collisions...
//		-----------------
int C, V, other_object;
...
//	Utilitie...
//	-----------
Q ground_round = 0, walls_of = 0, f_abs = 0;
...
void deadly_idof(int object) {
    ...
    if ((walls_of > 0) || (ground_round > A[object][2][0] - I[object][27])) {
        ...
        EDMS_object_collision(on2ph[object], on2ph[object], -9999,
                              (TFF.DATA1) * (ground_round > A[object][2][0] - I[object][27]),
                              (TFF.DATA2) * (walls_of > 0), location);
    ...
    //	--------------------------------------------------
    //	S[object][3][2] = -.5*( e1*T_beta_temp + e2*T_alpha_temp + e3*T_gamma_temp + ed1*beta_dot + ed2*alpha_dot +
    //ed3*gamma_dot );
    //			+ lagrange_multiplier*( ed0*lagrange + e0*lagrange_dot );

    //	S[object][4][2] =  .5*( e0*T_beta_temp + e2*T_gamma_temp - e3*T_alpha_temp + ed0*beta_dot + ed2*gamma_dot -
    //ed3*alpha_dot );
    //			+ lagrange_multiplier*( ed1*lagrange + e1*lagrange_dot );

    //	S[object][5][2] =  .5*( e0*T_alpha_temp + e3*T_beta_temp - e1*T_gamma_temp + ed0*alpha_dot + ed3*beta_dot -
    //ed1*gamma_dot );
    //			+ lagrange_multiplier*( ed2*lagrange + e2*lagrange_dot );

    //	S[object][6][2] =  .5*( e1*T_alpha_temp - e2*T_beta_temp + e0*T_gamma_temp + ed0*gamma_dot + ed1*alpha_dot -
    //ed2*beta_dot );
...
int make_death(Q init_state[6][3], Q params[10]) {
    ...
    if (object_number < MAX_OBJ) {
        ...
        //		To be sure...
        //		=============
        S[object_number][3][0] = S[object_number][4][0] = S[object_number][5][0] = END;
