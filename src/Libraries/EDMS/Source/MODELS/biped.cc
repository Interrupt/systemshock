...
void place_right_knee(int), place_left_knee(int);
...
//	Functions...
//	============
extern void (*idof_functions[MAX_OBJ])(int), (*equation_of_motion[MAX_OBJ][7])(int), null_function(int dummy);

extern Q *utility_pointer[MAX_OBJ];

//	Control Storage...
//	==================
Q control_x_t = 0, control_y_t = 0, control_gamma_t = 0;
...
//	============================================================
Q save_r_w_r[MAX_OBJ][3], save_r_w_l[MAX_OBJ][3], save_chi[MAX_OBJ];

//	Constants...
//	============
static Q pi = 3.1415, two_pi = 6.2831, point_five = .5, point_two_five = .25, point_nine = 0.9, two = 2., four = 4.,
         point_oo_one = .001, point_oo_five = .005;
...
//	Counters...
//	===========
int not_yet1[MAX_OBJ], not_yet2[MAX_OBJ], not_yet3[MAX_OBJ];
...
void biped_idof(int object) {
    ...
    R_w_r[0] += A[object][0][0];
    R_w_l[0] += A[object][0][0]; // STATE!
    R_w_r[1] += A[object][1][0];
    R_w_l[1] += A[object][1][0];
    R_w_r[2] += A[object][2][0];
    R_w_l[2] += A[object][2][0];

    //	Here's the local displacement...
    //	--------------------------------
    delta_r[0] = R_w_r[0] - I[object][3];
    delta_l[0] = R_w_l[0] - I[object][6];
    delta_r[1] = R_w_r[1] - I[object][4];
    delta_l[1] = R_w_l[1] - I[object][7];
    delta_r[2] = R_w_r[2] - I[object][5];
    delta_l[2] = R_w_l[2] - I[object][8];
    ...
    if (not_yet1[object]++ == 0) { // Multi-pass integration, ya know...
        save_r_w_r[object][0] = R_w_r[0];
        save_r_w_l[object][0] = R_w_l[0];
        save_r_w_r[object][1] = R_w_r[1];
        save_r_w_l[object][1] = R_w_l[1];
        save_r_w_r[object][2] = R_w_r[2];
        save_r_w_l[object][2] = R_w_l[2];
    }

    else {
        not_yet1[object] = 0;
        I[object][3] = save_r_w_r[object][0];
        I[object][6] = save_r_w_l[object][0];
        I[object][4] = save_r_w_r[object][1];
        I[object][7] = save_r_w_l[object][1];
        I[object][5] = save_r_w_r[object][2];
        I[object][8] = save_r_w_l[object][2];
    }

    //	Now we need the magnitudes of the deltas...
    //	-------------------------------------------
    mag_delta_r = sqrt(delta_r[0] * delta_r[0] + delta_r[1] * delta_r[1] // Walking only!
                       + delta_r[2] * delta_r[2]);

    mag_delta_l = sqrt(delta_l[0] * delta_l[0] + delta_l[1] * delta_l[1] + delta_l[2] * delta_l[2]);
    ...
    if (radius_r == 0)
        ...
        delta_chi = 2 * mag_delta_r / radius_r; // Angle...

    //	Need these for the actual placement...
    //	--------------------------------------
    if (abs(delta_chi) < point_oo_five)
        radius_l = 0;
    else
        radius_l = 2 * mag_delta_l / delta_chi;
    //	radius_l = radius_r;
    ...
    terrain_r.fix_to(TFF.g_height);
    terrain_r -= A[object][2][0];
    ...
    terrain_l.fix_to(TFF.g_height);
    terrain_l -= A[object][2][0];
    terrain_l_dx.fix_to(TFF.g_dx);
    terrain_l_dy.fix_to(TFF.g_dy);

    fc_r = fc_l = 0; // How are your Dr. Scholls...

    if (foot_r[2] <= terrain_r) {
        foot_r[2] = terrain_r;
        fc_r = 1;
    }
    if (foot_l[2] <= terrain_l) {
        foot_l[2] = terrain_l;
        fc_l = 1;
    }
    ...
    //	Yes! Yes! There it is...
    //	========================
    foot_r[0] = H_r[0] - foot_r[0];
    foot_l[0] = H_l[0] - foot_l[0];
    foot_r[1] = H_r[1] - foot_r[1];
    foot_l[1] = H_l[1] - foot_l[1];
    foot_r[2] = H_r[2] - foot_r[2];
    foot_l[2] = H_l[2] - foot_l[2];

    mag_foot_r = sqrt(foot_r[0] * foot_r[0] + foot_r[1] * foot_r[1] + foot_r[2] * foot_r[2]);

    mag_foot_l = sqrt(foot_l[0] * foot_l[0] + foot_l[1] * foot_l[1] + foot_l[2] * foot_l[2]);
    ...
    foot_hat_r[0] = normal_r * foot_r[0];
    foot_hat_l[0] = normal_l * foot_l[0];
    foot_hat_r[1] = normal_r * foot_r[1];
    foot_hat_l[1] = normal_l * foot_l[1];
    foot_hat_r[2] = normal_r * foot_r[2];
    foot_hat_l[2] = normal_l * foot_l[2];

    //	Find some damping...
    //	--------------------
    damp_r = dF_r[0] * foot_hat_r[0] + dF_r[1] * foot_hat_r[1] + dF_r[2] * foot_hat_r[2];

    damp_l = dF_l[0] * foot_hat_l[0] + dF_l[1] * foot_hat_l[1] + dF_l[2] * foot_hat_l[2];
    ...
    left_foot_mag = fc_l * (I[object][21] * ((I[object][0] - mag_foot_l)) + I[object][22] * damp_l);
    ...
    xi_beta = -I[object][28] * A[object][4][0] +
              .05 * Vl[1]; // + .03*beta_dot;		//Include velocity anticipation!!!!
    ...
    //	T_beta_t  = -I[object][26]*I[object][27]*I[object][29]				//What small displacement
    //limits??
    ...
    //	====================================
    F_right[0] = right_foot_mag * foot_hat_r[0];
    F_left[0] = left_foot_mag * foot_hat_l[0];
    F_right[1] = right_foot_mag * foot_hat_r[1];
    F_left[1] = left_foot_mag * foot_hat_l[1];
    F_right[2] = right_foot_mag * foot_hat_r[2];
    F_left[2] = left_foot_mag * foot_hat_l[2];
    ...
    if (abs(I[object][18]) < 0.01)
        control_x_t -= Vl[0] * 50 * (1 - ground_info);
    if (abs(I[object][19]) < 0.01)
        control_y_t -= Vl[1] * 50 * (1 - ground_info);
    ...
    //	-----------------------------------------------
    T_control_beta = .002 * (right_foot_mag * mag_foot_r * control_x_t + left_foot_mag * mag_foot_l * control_x_t);
    ...
    sec_alpha = 1. / cos_alpha; // oh, so slow...
    ...
    Q checker = sqrt(object0 * object0 + object1 * object1);

    if (checker > 0)
        checker = fix_one / checker; // To get primitive...
    else
        checker = 0;
...
void place_right_knee(int object) {
    ...
    Q alpha, beta, h, r, m, rho, mu, k_long;
    ...
    utility_pointer[object][2] = utility_pointer[object][0] * terrain_r_dx + utility_pointer[object][1] * terrain_r_dy;
    //	if ( fc_r == 0 ) utility_pointer[object][2] += .5*foot_length;

    localize(utility_pointer[object][0], utility_pointer[object][1], utility_pointer[object][2]);
    ...
    //	--------------------------------
    h = -.5 * sqrt((alpha * alpha - mag_foot_r * mag_foot_r) * (mag_foot_r * mag_foot_r - beta * beta)) /
        (mag_foot_r + .01);

    //	Now we need the unit vector in the leg basis in that direction...
    //	-----------------------------------------------------------------
    m = -foot_hat_r[0] * utility_pointer[object][0] - foot_hat_r[1] * utility_pointer[object][1];
...
void place_left_knee(int object) {
    ...
    utility_pointer[object][5] = utility_pointer[object][3] * terrain_l_dx + utility_pointer[object][4] * terrain_l_dy;
    //	if ( fc_l == 0 ) utility_pointer[object][5] += .5*foot_length;

    localize(utility_pointer[object][3], utility_pointer[object][4], utility_pointer[object][5]);
    ...
    //	Here's the knee perpendicular...
    //	--------------------------------
    h = -.5 * sqrt((alpha * alpha - mag_foot_l * mag_foot_l) * (mag_foot_l * mag_foot_l - beta * beta)) /
        (mag_foot_l + point_oo_one);

    //	Now we need the unit vector in the leg basis in that direction...
    //	-----------------------------------------------------------------
    m = -foot_hat_l[0] * utility_pointer[object][3] - foot_hat_l[1] * utility_pointer[object][4];
