  ...
  move.l 4(sp),
      d1 // check hi
          bne.s must_use_quad
              ...
              move.l 8(sp),
      -(sp)jsr long_sqrt addq.w #4,
      sp rts

          must_use_quad : move.l 8(sp),
      d0 // low
              movem.l d3 -
          d6,
      -(sp) // save regs
      ...
      a0 // get table address
          ...
          q_test_A : swap d1 // ror	edx,16

                         move.w d1,
      d3 andi.w #0xff00,
      d3                      // or	dh, dh		       // check high 8 bits of eax
          bmi.s q_return_zero // js	return_zero	       // arg is
                              // negative!
              beq.s q_test_B  // jz	q_test_B
                  ror.w #8,
      d3 move.b d3,
      d1 // mov	al, dh		       // prepare for xlat
              move.w #16 +
          12,
      d2                         // mov	cl, 16+12
      ...
      d2                     // mov	cl, 16+8
          bra.s q_found_bits // jmp	q_found_bits
              ...
              q_test_C : swap d1 // ror	edx,16

                             move.w d1,
      d3 andi.w #0xff00,
      d3                 // or	dh, dh		       // check next 8 bits of eax
          beq.s q_test_D // jz	q_test_D
              ror.w #8,
      d3 move.b d3,
      d1 // mov	al, ah		       // prepare for xlat
              move.w #16 +
          4,
      d2                     // mov	cl, 16+4
          bra.s q_found_bits // jmp	q_found_bits
      ...
      : moveq #0,
        ...
        d3 //	xlat
           // movzx	ebx, al
            asl.l d2,
        d3 // shl	ebx, cl		       // bx now has the first guess

            //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

            // experience has shown that we almost always go through the loop
            // just about three times.  to avoid compares and jumps, we iterate
            // three times without even thinking about it, and then start
            // checking if our answer is correct.

            q_newton_loop : move.l d5,
        d0 move.l d6,
        d1 dc
            .l 0x4C430401 //  divu.l	d3,d1:d0
        add.l d0,
        d3 roxr.l #1,
        d3
        ...
        d3
            ...
            q_newton_loop2 : move.l d5,
        d0 move.l d6,
        d1 dc
            .l 0x4C430401 //  divu.l	d3,d1:d0
        cmp.l d0,
        d3                        // cmp	eax, ebx
            beq.s q_bx_is_correct // je	q_bx_is_correct	       // if bx =
                                  // ax, we win
                move.l d3,
        d2 // mov	ecx, ebx		       // save old bx for
           // comparing
            add.l d0,
        d3 // add	ebx, eax
            roxr.l #1,
        d3 // rcr	ebx, 1		       // bx <- (bx+ax)/2
            cmp.l d0,
        d3                      // cmp	eax, ebx		       // compare old ax with
                                // average
            beq.s q_bx_is_close // je	q_bx_is_close
                cmp.l d2,
        d3                      // cmp	ecx, ebx		       // compare old bx with
                                // average
            beq.s q_bx_is_close // je	q_bx_is_close
                bra.s q_newton_loop2 // jmp	q_newton_loop2

                    // now we must find which is closer to x, bx^2 or
                    // (bx+1)^2 x = bx*(bx+1)+r = bx^2 + bx + r
                    //   which is closer to bx^2 than to bx^2 + 2bx + 1
                    //   only if r = 0!  luckily we have r in dx from the
                    //   divide
                    q_bx_is_close
      : tst.l d1                  // or	edx,edx
      ...
      ,
        d3 - d6      // restore regs
                 rts // ret
                     ...
                     q_return_zero : moveq #0,
        d0 // sub	eax, eax	       // return 0
                movem.l(sp) +
      ,
        d3 - d6      // restore regs
                 rts // ret
...
asm long long_sqrt(long num) {
  move.l 4(sp),
      ...
      d1 // mov	edx, eax
          swap
              d1 // ror	edx, 16		       // get high 16 bits of arg in dx
      ...
      d3 // or	dh, dh		       // check high 8 bits of eax
          bmi.s
              return_zero      // js	return_zero	       // arg is negative!
      ...
      d2                   // mov	cl, 8
          bra.s found_bits // jmp	found_bits
              ...
              test_C : move.w d0,
      d3 andi.w #0xff00,
      d3               // or	ah, ah		       // check next 8 bits of eax
      ...
      d2 // sub	cl, cl
          ...
          found_bits : andi.l #0x00ff,
      d1 // clear high bytes
          move.b(a0, d1.w),
      d1 // xlat
         // movzx	ebx, al
          asl.l d2,
      d1 // shl	ebx, cl		       // bx now has the first guess
          ...
          newton_loop : move.l d5,
      d0 divu.w d1, d0 add.w d0, d1 roxr.w #1,
      d1
      ...
      d1
          ...
          newton_loop2 : move.l d5,
      d0 divu.w d1, d0 cmp.w d0,
      d1                      // cmp	ax, bx
          beq.s bx_is_correct // je	bx_is_correct	       // if bx =
                              // ax, we win
              move.w d1,
      d2 // mov	cx, bx		       // save old bx for comparing
          add.w d0,
      d1 // add	bx, ax
          roxr.w #1,
      d1 // rcr	bx, 1		       // bx <- (bx+ax)/2
          cmp.w d0,
      d1                    // cmp	ax, bx		       // compare old ax with average
          beq.s bx_is_close // je	bx_is_close
              cmp.w d2,
      d1                    // cmp	cx, bx		       // compare old bx with average
          beq.s bx_is_close // je	bx_is_close
              bra.s newton_loop2
                  ...
                  bx_is_close : swap d0 tst.w d0 beq.s bx_is_correct addq.l #1,
      d1

          bx_is_correct : move.l d1,
      d0 // mov	eax, ebx
              movem.l(sp) +
      ,
      d3 - d5      // restore regs
