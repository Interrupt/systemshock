...
fix24 fix24_sin(fixang theta) { return (fix_sin(theta) >> 8); }

fix24 fix24_cos(fixang theta) { return (fix_cos(theta) >> 8); }
...
fixang fix24_acos(fix24 x) { return (fix_acos(x << 8)); }
...
fixang fix24_atan2(fix24 y, fix24 x) {
  ...
  // Use fix24_asin or fix24_acos depending on where we are.  We don't want to
  // use fix24_asin if the sin is close to 1 or -1
  s = fix24_div(y, hyp);
  if ((ulong)s < 0x00004000 ||
      (ulong)s > 0xffffc000) { // range is good, use asin
    th = fix24_asin(s);
    if (x < 0) {
      if (th < 0x4000)
        th = 0x8000 - th;
