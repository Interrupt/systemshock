...
errtype hotkey_add(ushort keycode, ulong contexts, hotkey_callback func, intptr_t state)
{
#ifdef HOTKEY_HELP
   return(hotkey_add_help(keycode,contexts,func,state,NULL));
}

errtype hotkey_add_help(ushort keycode, ulong contexts, hotkey_callback func, intptr_t state, char * help_text)
{
#endif
   hotkey_entry e,*ch;
   errtype err;
