...
void gri_opaque_clut_per_umap_hscan_scanline(grs_per_info *pi, grs_bitmap *bm) {
  ...
  if (l_x < l_xl) {
    ...
    for (; l_x < l_xl; l_x++) {
      if (test <= 0) {
        ...
        *p = t_clut
            [bm_bits[k]]; // gr_fill_upixel(t_clut[bm_bits[k]],l_x,y_cint);
      }
      temp_y = y_cint;
  ...
  for (; l_x < l_xr0; l_x++) {
    ...
    k += (l_v >> l_v_shift) & l_v_mask;
    *(p++) =
        t_clut[bm_bits[k]]; // gr_fill_upixel(t_clut[bm_bits[k]],l_x,y_cint);
    temp_y = y_cint;
    y_cint = fix_int(l_y_fix += l_scan_slope);
    if (temp_y != y_cint)
      p += gr_row;
  ...
  if (l_y < l_yl) {
    ...
    for (; l_y < l_yl; l_y++) {
      if (test <= 0) {
        ...
        k += (l_v >> l_v_shift) & l_v_mask;
        *p = t_clut
            [bm_bits[k]]; // gr_fill_upixel(t_clut[bm_bits[k]],x_cint,l_y);
      }
      temp_x = x_cint;
