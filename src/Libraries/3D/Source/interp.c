...
#include "GlobalV.h"
#include "lg.h"
//#include <stdarg.h> //(DER) NOT USED:  va_list  va_start
...
// ptr to stack parms
ubyte *parm_ptr; //(DER) WAS:  va_list parm_ptr;
...
void g3_interpret_object(ubyte *object_ptr, ...)
{
  ...
  // get addr of stack parms, stdarg style
  parm_ptr = (&object_ptr) + sizeof(object_ptr); //(DER) WAS:  va_start(parm_ptr, object_ptr);
  ...
  if (scale)
  {
    ...
    else
    {
      ...
      if (((temp << scale) && 0xffff0000) != 0) return; // overflow
...
uchar *do_eof(uchar *opcode) { return 0; } // and return extra level
...
uchar *do_setshade(uchar *opcode)
{
  ...
  while (--i >= 0)
  {
    g3s_phandle temphand = resbuf[*(unsigned short *)(opcode + 4 + (i << 2))]; // get point handle
...
uchar *do_rgbshades(uchar *opcode)
{
  ...
  while (--i >= 0)
  {
    g3s_phandle temphand = resbuf[*(unsigned short *)new_opcode]; // get point handle
...
uchar *do_uvlist(uchar *opcode)
{
  ...
  while (--i >= 0)
  {
    g3s_phandle temphand = resbuf[*(unsigned short *)opcode]; // get point handle
...
uchar *do_tmap_op(uchar *opcode)
{
  ...
  do
  {
    short temp = *(short *)(opcode + 6 + (count << 1));
...
uchar *do_ldjnorm(uchar *opcode)
{
  if (g3_check_normal_facing((g3s_vector *)(opcode + 16), (g3s_vector *)(opcode + 4)))
  {
    fix temp = g3_vec_dotprod(&_g3d_light_vec, (g3s_vector *)(opcode + 4));
