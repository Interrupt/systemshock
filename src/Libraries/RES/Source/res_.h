...
ptr to cumulative stats by id extern ResCumStat
    cumStatType[NUM_RESTYPENAMES]; // table of cum. stats by type
...
typedef struct {
  ...
  across calls uint32_t totSizeGotten; // total # bytes paged out
  across calls
} ResPageStats;
...
prototypes void ResSpewCumStats(); // these are in rescum.c
